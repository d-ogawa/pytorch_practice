...
class Net(nn.Module):
    ...
    #     self.conv2_drop = nn.Dropout2d()            # default droprate p=0.5
    ...
    def forward(self, x):
        ...
        # or, default
        # x = F.relu(F.max_pool2d(self.conv2_drop(self.conv2(x), p=0.2), 2))
        ...
        # 512 -> 128
        x = F.relu(self.fc1(x))
        x = F.dropout(x, p=0.2, training=self.training) # default droprate p=0.5
...
def main():
    ...
    train_loader = DataLoader(
                        myDataset(path_to_dir="../data/mnist_train_pt",
                                  transform=transforms.Compose([
                                    transforms.Normalize((0.1307, ), (0.3081, ))
                                    ])),
                        batch_size=args.batch_size,
    ...
    test_loader = DataLoader(
                        myDataset(path_to_dir="../data/mnist_test_pt",
                                  transform=transforms.Compose([
                                    transforms.Normalize((0.1307, ), (0.3081, ))
                                    ])),
                        batch_size=args.test_batch_size,
